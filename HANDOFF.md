...
- `GEMINI_API_KEY` — set in Vercel dashboard (Settings → Environment Variables).
  Never in the repo, never in client code. `api/riddle.js` reads it from `process.env`.
  The reply is generated by Google's Gemini model, chosen for its free tier so the
  diary runs at no cost. Swapping providers is a change to `api/riddle.js` alone — the
  front-end illusion never touches the model.
