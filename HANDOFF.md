...
- `GROQ_API_KEY` — set in Vercel dashboard (Settings → Environment Variables).
  Never in the repo, never in client code. `api/riddle.js` reads it from `process.env`.
  The reply is generated by a model on Groq's free tier (chosen after Gemini's free
  tier proved too small — it 429'd almost immediately). Swapping providers is a change
  to `api/riddle.js` alone — the front-end illusion never touches the model.
- `FIREBASE_SERVICE_ACCOUNT` — the Firestore service-account JSON (persistence). Optional:
  without it the diary still replies, it just forgets between visits.
